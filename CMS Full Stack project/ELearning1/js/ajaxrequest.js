$(document).ready(function() {
  ...
  // Checking Phone No. on keypress
  $("#stuphone").keypress(function() {
...
function addStu() {
  ...
  if (stuname.trim() === "") {
  ...
  } else if (stuphone.trim() !== "" && !reph.test(stuphone)) {
    $("#statusMsg4").html(
      '<small style="color:red;"> Please Enter Valid Mobile No. </small>'
    );
    $("#stuemail").focus();
    return false;
    ...
    $("#stupass").focus();
    return false;
  } else if ( !rep.test(stupass) ) {
    $("#statusMsg3").html(
      '<small style="color:red;"> Password too Short ! at least 8 characters required !</small>'
    );
    $("#stupass").focus();
    return false;
  } else {
